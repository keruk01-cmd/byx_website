...
전체 2,140줄. 줄 번호는 대략치이니 섹션 주석(`/* ==== N. NAME ==== */`)으로 찾으세요.
...
색을 고르는 기능은 일부러 넣지 않았습니다. 팔레트가 네 토큰뿐이라(§4-1)
"컬러"에 해당하는 건 `==강조==` 의 흑백 반전과 `((흐리게))` 두 가지입니다.

이미지는 `＋ 이미지` 버튼이나 **텍스트 칸에 드래그&드롭** → 업로드 후 `insertAtCaret()` 이
커서 자리에 표기를 넣습니다. 툴바는 `wrapSelection()`(선택 감싸기)과
`prefixBlock()`(커서가 있는 블록에만 토큰 붙이기)을 씁니다.
**셋 다 재렌더를 하지 않습니다** — 하면 커서와 포커스가 날아갑니다.
...
| 33 | 색 선택 기능은 넣지 않음 | 팔레트가 네 토큰(§4-1). 임의 색을 열면 시스템이 무너짐. `==강조==` 반전과 `((흐리게))` 로 대체 |
